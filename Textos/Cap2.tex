...
O objetivo desse capítulo é fornecer uma base para a leitura do texto apresentado nessa monografia em que são estudadas as tecnologias assistivas, a visão computacional e as formas de identificação de objetos em imagem e é proposto um formato para reconhecer mãos em vídeo.
...
Visão computacional estuda como reconstruir, interromper e compreender uma cena 3D utilizando termos das propriedades da estrutura através de imagens 2D \citeonline{visao_computacional}. O termo é frequentemente utilizado como sinônimo de reconhecimento de imagem, porém visão computacional vai além de analisar imagens. O reconhecimento de imagens faz parte do processo de visão computacional, que engloba desde reconhecimento de objetos e caracteres, até analise de textos e sentimentos.

Para um computador reconhecer e entender uma cena, segundo \citeonline{kaiser}, são seguidos alguns passos.

\begin{itemize}
    \item Aquisição de Imagem
    \item Processamento da Imagem
    \item Análise da imagem
\end{itemize}

Aquisição de imagem, é um processo de captura de imagens,capturada tanto por vídeos quanto por fotos. Geralmente essas imagens capturadas, passam por um tratamento para melhorar a qualidade e minimizar erros no processamento. 

O processamento da imagem é feito em baixo nível, onde é incluído detecção de borda, segmentação e classificação.

Na análise da imagem, é feita a tomada de decisão, onde são aplicados algoritmos de alto nível usando os dados coletados nos passos anteriores.
